...
election_config <- tribble(
  ...
  2017, "cvr/frey_vs_dehn_by_precinct.csv", "MN-shapefiles/MN16/mn_precincts16.shp", "Raymond Dehn_votes", "Dehn",
  2021, "cvr/frey_vs_knuth_by_precinct.csv", "mpls_precincts_2021/general_election_results_by_precinct_2020.shp", "Kate Knuth_votes", "Knuth",
  2025, "cvr/fateh_vs_frey_by_precinct.csv", "mpls_precincts_2025/bdry_votingdistricts.shp", "Omar Fateh_votes", "Fateh"
)
...
for (i in seq_len(nrow(election_results))) {
  ...
  ggsave(
    filename = paste0("plots/", year, "_frey_share.png"),
    plot = vote_share_map,
    width = 10,
    height = 8,
    dpi = 300
  ...
  print(lisa_map)
  ggsave(
    filename = paste0("plots/", year, "_lisa_clusters.png"),
    plot = lisa_map,
    width = 10,
    height = 8,
    dpi = 300
...
# Output the sf object for each election into a shapefile

# Save the spatial data for each election year as shapefiles in a "shapefiles" directory
...
for (i in seq_len(nrow(election_results))) {
  year <- election_results$year[i]
  spatial_data <- election_results$spatial_data[[i]]
  shapefile_path <- paste0("shapefiles/election_", year, ".shp")
  st_write(spatial_data, shapefile_path, delete_layer = TRUE, quiet = TRUE)
  cat("Shapefile written to:", shapefile_path, "\n")
}
