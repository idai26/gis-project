...
st_write(mpls_tracts_sf, "../data/geopackages/mpls_tracts_census.gpkg", delete_dsn = TRUE)
...
st_write(mpls_tracts_sf |> select(1:50), "../data/shapefiles/tracts/mpls_tracts_census.shp", 
         delete_layer = TRUE)

# Save tabular data as CSV (without geometry)
mpls_tracts_sf |>
  st_drop_geometry() |>
  write_csv("../data/processed/census/mpls_tracts_census.csv")

cat("\nData saved to:\n")
cat("  - ../data/geopackages/mpls_tracts_census.gpkg (GeoPackage with all variables)\n")
cat("  - ../data/shapefiles/tracts/mpls_tracts_census.shp (Shapefile subset)\n")
cat("  - ../data/processed/census/mpls_tracts_census.csv (CSV without geometry)\n")
