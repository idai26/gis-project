...
if (!dir.exists("lisa_plots")) {
  dir.create("lisa_plots")
}

# Load tract shapefile
tract_sf <- st_read("tract_election_shapefiles/mpls_tract_elections.shp", quiet = TRUE) |>
  st_transform(26915)

# Load tract-level election results
election_results <- read_csv("tract_election_results_all.csv", show_col_types = FALSE) |>
...
for (var_name in pct_columns) {
  ...
  # Save plot
  filename <- paste0("lisa_plots/tract_", var_name, "_lisa.png")
  ggsave(
    filename = filename,
    plot = lisa_map,
    width = 10,
...
# Save summary to CSV
write_csv(summary_results, "lisa_plots/tract_lisa_summary.csv")
cat("\nSummary saved to: lisa_plots/tract_lisa_summary.csv\n")
