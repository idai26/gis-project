...
The only significant coefficient in the SEM model is *pct_built_post2000*, which is positively correlated with Frey's vote share. One of the most impactful policies adopted under Frey's tenure has been the Minneapolis 2040 Comprehensive Plan, which opened up nearly the entire city to multifamily housing development and created ambitious affordable housing goals. Despite the strong pushback from some residents, it appears that areas that have seen more housing development during Frey's tenure are more supportive of him. Of course, we cannot determine the causal relationship between housing development and support for Frey. It is possible that areas that have seen more housing development during Frey's tenure are also the most economically prosperous and thus able to attract new development, giving them good reason to support him. On the other hand, Frey could also be directing resources to areas most supportive to him, reversing the causality.

Future research might consider using a spatial lag model to account for the spatial correlation of the dependent variable. Due to the interpolation of election results to census tracts, there is the possibility that spatial diffusion in Frey's vote share would have been created artificially. As a result, I decided not to include a spatial lag model in this analysis. Using only precinct-level data from a single election might be a possible way deterine if one precicnt's support for Frey is significantly correlated with the support for Frey in its surrounding precincts, after accounting for the demographic trends in the surrounding precincts.
